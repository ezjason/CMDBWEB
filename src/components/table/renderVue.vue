        ...
        data() {
            return {
                ...
                btnIcon:{
                    ...
                    '编辑HTTP数据集':'icon-Group1',
                    '编辑SQL数据集':'icon--charusql',
                    '新建数据集':'icon-shujuji',
                    '表详情':'icon-baobiao',
                },
            ...
            linkClick(scope,column){
                switch(column.link.type){
                    case 'pageOpen':
                        ...
                        break;
                    default:
                        let vue=this.formatOption(column.link.vue,scope.row);
                        if(vue){
                            this.linkBox.toggle({
                                title: this.formatOption(column.link.title,scope.row)||scope.row[column.colkey],
                                data: {
                                    ...scope,
                                    selfVue:vue,
                                    data:this.formatOption(column.link.data,scope.row)
